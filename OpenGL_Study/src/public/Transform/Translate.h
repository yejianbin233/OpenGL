...
bool left_or_right_translate; // 左右移动标记，true 为左，false 为右
float translate_offset=0.0f; // 位移量
float translate_max_offset=0.7f; // 最大位移量
float translate_offset_increment=0.0005f;; // 位移增量

namespace TranslateRendering
{
    static int TranslateRenderingRun()
    {
        ...
        *   layout (location = 0) in vec3 aPos;
        ...
        while (!glfwWindowShouldClose(window))
        {
            ...
            // -- 位移数据计算 --
            if (left_or_right_translate)
            {
                translate_offset -= translate_offset_increment;
            }
            else
            {
                translate_offset += translate_offset_increment;
            }
            if (abs(translate_offset) >= translate_max_offset)
            {
                translate_offset = (translate_offset / abs(translate_offset)) * translate_max_offset; // 限制最大位移
                left_or_right_translate = !left_or_right_translate; // 反转方向
            }
            
            // -- 位移数据计算 --
            // ourShader.use(); // 使用着色器
            glm::mat4 transform = glm::mat4(1.0f); // make sure to initialize matrix to identity matrix first
            transform = glm::translate(transform, glm::vec3(translate_offset, 0.0f, 0.0f));

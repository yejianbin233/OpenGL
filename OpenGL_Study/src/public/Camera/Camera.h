...
// 使用索引定义多个面，可形成立体。
namespace CameraRendering
{
    static int CameraRenderingRun()
    {
        ...
        // tell GLFW to capture our mouse - 鼠标捕获
        ...
        *   layout (location = 0) in vec3 aPos;
        ...
        // -- 着色器（Shader） --
        Shader ourShader("src/Shaders/VertexShaders/Camera.vs", "src/Shaders/FragmentShaders/Indice.fs");
        ourShader.use();
        // -- 着色器（Shader）  --
        ...
        while (!glfwWindowShouldClose(window))
        {
            ...
            ourShader.use();
            // -- 摄像机 --
            // 初始化摄像机属性后，每帧根据摄像机的变化来更新 MVP 变换（模型-视图-投影变换）。

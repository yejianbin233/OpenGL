...
// 使用索引定义多个面，可形成立体。
namespace FaceCullingRendering
{
    static int FaceCullingRenderingRun()
    {
       // glfw: initialize and configure
        ...
        glEnable(GL_CULL_FACE); // 开启"面剔除"
        glCullFace(GL_BACK); // "面剔除"指定剔除"背面"
        glFrontFace(GL_CW); // "面剔除"指定正面模式（GL_CW），代表顺时针为正面
        ...
        // -- 着色器（Shader） --
        Shader lightingShader("src/Shaders/VertexShaders/Lighting/LightingSource/StandardLighting_PointLight.vs", "src/Shaders/FragmentShaders/Lighting/LightingSource/StandardLighting_PointLight.fs");
        // -- 着色器（Shader）  --
